...
from metric import *
from model.GANet_ms import GANet
...
best_iou = 0
ckpt_path = 'checkpoint/GANet.ckpt'
...
def train(model, dataset, loss_fn, optimizer, met):
    ...
    def forward_fn(data, label):
        logits1,logits2,logits3,logits4,logits5,logits6 = model(data)
        ...
        loss6 = loss_fn(logits6, label)
        return (loss1, loss2,loss3,loss4,loss5,loss6), (logits1, logits2,logits3,logits4,logits5,logits6)
...
def val(model, dataset, loss_fn, met):
    ...
    val_label = []
    for batch, (data, label) in enumerate(dataset.create_tuple_iterator()):
        pred1,pred2,pred3,pred4,pred5,pred6 = model(data)
        ...
        val_loss += loss_fn(pred6, label).asnumpy()
