...
def calc_confusion_mat(D, Y):
    FP = (D != Y) & (Y.astype(np.bool_) == False)
    FN = (D != Y) & (Y.astype(np.bool_) == True)
    TN = (D == Y) & (Y.astype(np.bool_) == False)
    TP = (D == Y) & (Y.astype(np.bool_) == True)
...
def evaluate_metrics(samples, results_path, run_name):
    ...
    print(
        f'{run_name} EVAL AUC={metrics["AUC"]}, and AP={metrics["AP"]}, w/ best thr={metrics["best_thr"]} at f-m={metrics["best_f_measure"]} and FP={sum(metrics["FP"])}, FN={sum(metrics["FN"])}')

    with open(os.path.join(results_path, 'metrics.pkl'), 'wb') as f:
        pickle.dump(metrics, f)
        f.close()
    print(
        f'{run_name} EVAL AUC={float(metrics["AUC"]):f}, and AP={float(metrics["AP"]):f}, w/ best thr={float(metrics["best_thr"]):f} at f-m={float(metrics["best_f_measure"]):.3f} and FP={int(sum(metrics["FP"])):d}, FN={int(sum(metrics["FN"])):d}')
    plt.figure(1)
    plt.clf()
    plt.plot(metrics['recall'], metrics['precision'])
    plt.title('Average Precision=%.4f' % float(metrics['AP']))
    plt.xlabel('Recall')
    ...
    plt.title('AUC=%.4f' % float(metrics['AUC']))
